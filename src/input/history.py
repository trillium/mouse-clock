...
_global_history = PositionHistory()


def push_position(position: Tuple[float, float]):
    """Add position to global history stack."""
    _global_history.push_position(position)


def pop_position() -> Optional[Tuple[float, float]]:
    """Remove and return last position from global history."""
    return _global_history.pop_position()


def peek_position() -> Optional[Tuple[float, float]]:
    """View last position without removing from global history."""
    return _global_history.peek_position()


def clear_history():
    """Clear global position history."""
    _global_history.clear_history()


def get_origin() -> Optional[Tuple[float, float]]:
    """Get session origin from global history."""
    return _global_history.get_origin()


def set_origin(position: Tuple[float, float]):
    """Set session origin in global history."""
    _global_history.set_origin(position)


def get_history_instance() -> PositionHistory:
