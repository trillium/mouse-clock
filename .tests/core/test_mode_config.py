...
class TestGetModeConfig:
    ...
    def test_falls_back_to_global_colors(self):
        ...
        result = config.get_mode_config("clock_letters", "colors")
        assert result == config.get_active_colors()

    def test_falls_back_to_global_styles(self):
        result = config.get_mode_config("clock_letters", "styles")
        assert result == config.get_active_styles()

    def test_falls_back_to_global_letters(self):
        result = config.get_mode_config("clock_letters", "letters")
        assert result == config.get_active_letters()

    def test_returns_per_mode_when_set(self):
        config.set_mode_config("clock_letters", "colors", ["red", "blue"])
        assert config.get_mode_config("clock_letters", "colors") == ["red", "blue"]

    def test_per_mode_does_not_affect_other_modes(self):
        config.set_mode_config("clock_letters", "colors", ["red"])
...
class TestAddModeItem:
    ...
    def test_copies_global_on_first_write(self):
        ...
        config.add_mode_item("clock_letters", "colors", "green")
        result = config.get_mode_config("clock_letters", "colors")
    ...
    def test_rejects_invalid_item(self):
        assert config.add_mode_item("clock_letters", "colors", "neon") is False

    def test_rejects_duplicate(self):
        # All colors are active by default, so adding any should fail
        assert config.add_mode_item("clock_letters", "colors", "red") is False

    def test_returns_true_on_success(self):
        config.set_mode_config("clock_letters", "colors", ["red"])
        assert config.add_mode_item("clock_letters", "colors", "blue") is True
        assert config.get_mode_config("clock_letters", "colors") == ["red", "blue"]

    def test_add_style(self):
        config.set_mode_config("clock_letters", "styles", ["dash"])
        assert config.add_mode_item("clock_letters", "styles", "dot") is True
        assert "dot" in config.get_mode_config("clock_letters", "styles")

    def test_add_letter(self):
        config.set_mode_config("clock_letters", "letters", ["a", "b"])
        assert config.add_mode_item("clock_letters", "letters", "c") is True
        assert "c" in config.get_mode_config("clock_letters", "letters")
...
class TestRemoveModeItem:
    ...
    def test_remove_existing_item(self):
        ...
        result = config.remove_mode_item("clock_letters", "colors", "red")
        assert result is True
        assert "red" not in config.get_mode_config("clock_letters", "colors")

    def test_remove_nonexistent_returns_false(self):
        config.set_mode_config("clock_letters", "colors", ["red"])
        assert config.remove_mode_item("clock_letters", "colors", "blue") is False

    def test_remove_does_not_affect_global(self):
        config.remove_mode_item("clock_letters", "colors", "red")
...
class TestResetModeConfig:
    ...
    def test_restores_fallback(self):
        config.set_mode_config("clock_letters", "colors", ["red"])
        config.reset_mode_config("clock_letters", "colors")
        # Should fall back to global again
        assert config.get_mode_config("clock_letters", "colors") == config.get_active_colors()
...
class TestSetModeConfig:
    ...
    def test_set_full_list(self):
        config.set_mode_config("clock_letters", "colors", ["red", "green"])
        assert config.get_mode_config("clock_letters", "colors") == ["red", "green"]
